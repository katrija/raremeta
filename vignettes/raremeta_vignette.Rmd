...
Readers who are already familiar with other `R`-packages for meta analysis (e.g. **metafor** or **meta**) might recognize parts of the input syntax and have no problems to interpret the output.
...
## rarePeto
To conduct a meta-analysis on the log odds ration using Peto's method (see Yusuf et al., 1985), the `rarePeto` function can be used.  
...
Even though most functions allow to apply the correction internally, users may find it easier to pre-process the data this way and then input it to the other functions.
Since the functions above are set up in a way that a already continuity corrected data input will not be continuity corrected again, the user must not fear of applying it twice.  
...
The data input may be achieved by specifying an appropriate subset of the first 8 arguments.
We will go into more depth on the possibility to specifying the `x` argument later in this section.
The following 7 arguments reflect the information included in the contingency table representation above, with `data` specifying the data frame. 
...
The argument `drop00` is a  logical indicating whether double-zero studies should be excluded prior to calculating the studies’ effect sizes and sampling variances.    

Let us have a look at the output of an application of the `rareES` function. 
We calculate the log odds ration on the study level for the Yuan data set. 
```{r, eval=TRUE}
