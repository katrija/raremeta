...
work with the **raremeta** package for `R`. 
For a quick dive into the
workflow, we begin with an example interlaced with the corresponding
`R`-code. 
...
# A Tour Through the Functionality of `raremeta`

In the following section we are about to use a great deal of the implemented functionality 
on a data set by Yuan et. al. (2018).  
...
An example for such data is provided in a meta-analysis by Yuan  et al. (2018). 
In this meta-analysis, the authors evaluate the acceptability of bibliotherapy for depression and anxiety disorders in children and adolescents. 
Specifically, they examine whether bibliotherapy is associated with a larger (or smaller) number of dropouts compared to control treatments. 
A larger number of dropouts in bibliotherapy indicates a lower acceptability of this treatment.  

In the following, we show how these data can be analyzed with different models for meta-analysis of rare events using the `raremeta` package.
...
The data frame consists of 8 rows and 5 columns. 
Each of the rows stand for one individual study.
The first column, `study`, includes the study identifier, while the other columns, `ai, ci, n1i, n2i`, include information on the event counts and number of participants in the different studies. 
This information is usually depicted in form of a table  
...
where `ai,ci` and `n1i,n2i` are the number of events and the
total number of participants of the i'th study in the treatment
group and the control group, respectively.
In the Yuan data, the treatment is bibliotherapy.
This makes the treatment group consist of those individuals undergoing bibliotherapy and the control group consist of those undergoing the control treatment.
Usually, studies with no event in one of the groups are called single-zero studies, while studies with no event in both of the groups are called double-zero studies.
The meta-analysis by Yuan et al. is in fact an example for a meta-analysis of a rare event: Specifically, it contains two studies with no events in both groups (double-zero studies).  

We will now briefly show how to apply the inverse-variance method for the log odds ratio with the standard continuity correction (add 0.5 to all cells of all studies with zero counts) to the data.  

```{r, eval = TRUE}
fitIV <- rareIV(ai=ai,ci=ci,n1i=n1i,n2i=n2i,data=dat.yuan2018,
                measure="logOR",method="FE",cc="constant",ccval=0.5,ccto="only0")

fitIV
```

Readers who are already familiar with other `R`-packages for meta analysis (e.g. **metafor** or **meta**) might recognize parts of the input syntax and will likely have no problems to interpret the output.
We continue with an in depth account which explains the usage and output-interpretation of all major functions included in `raremeta`.
...
This may be done in `raremeta` by specifying the arguments `data, ai, bi, ci, and di` or `data, ai, n1i, ci, n2i`.
We will go into more depth on the possibility to specifying the `x` argument later in this section.  
...
The argument `method` specifies whether a fixed- or a random-effects model should be fitted. 
A fixed-effects model is fitted when using `method = "FE"` . 
A random-effects model is fitted by setting method equal to one of the following: `"DL", "HE", "SJ", "ML", "REML", "EB", "HS", "PM", "IPM", "GENQ", "PMM"` or `"GENQM"`.
The arguments `cc` to `ccto` define the type of continuity correction applied to the data before the analysis and we will go into more depth in its application in later parts.
The argument `drop00` specifies whether double zero studies shall be excluded from the analysis.  
For an interpretation of the additional arguments, consult the corresponding function documentation.  

If we use the Yuan data to fit a random effects meta-analysis on the logarithmised odds-ratio using the median-unbiased Paule-Mandel estimator while adding the constant value of 0.5 to all cells of all studies (as described in Viechtbauer 2021), we use the following input.
...
In this case, the total number of studies in the Yuan data is 8, while 6 studies were part of the analysis.
This stems from the fact, that, by default, the argument `drop00` was set to `TRUE`.    
The next part of the output reports the Q-statistic, p-value, heterogeneity estimate and I2-statistic.
The output ends with the estimated model parameters.
The logarithmised odd-ratio was estimated to be 0.5805 (implying a estimated odds ratio of 1.786963). 
Following the estimated effect size, the output also includes estimates for other model parameters: the estimators standard error, z-value, p-value, and the lower and upper bounds of the confidence interval (the confidence level was set to 95% by default).  


## rareMH
A prominent class of estimators for the common effect size under the the fixed-effects model was proposed by Mantel and Haenszel in 1959.
The `rareMH()` function enables the user to conduct a meta-analysis using Mantel-Haenszel type estimators.
...
Data input and argument interpretation work in the usual way (see the corresponding function documentation or the part concerning `rareIV` above for more information.)  
The additional argument `correct` allows the user to decide whether continuity correction shall be applied.
There is a difference between setting the options `correct=FALSE` and `cc="none"`.
While setting `correct=FALSE` implies no change of the data before estimation whatsoever, the option `cc="none"` excludes all single- (and double-) zero studies from the analysis.
As a quotient of sums rather than a sum of quotients, Mantel-Haenszel type estimators are particularly useful in meta-analysis of rare events (see e.g. sections 9.2-9.4 of Jewell (2013)).
In fact, for the estimation of the common effect size, no continuity correction is needed.  

Let us look at the application of `rareMH` in on our data set.
We aim to estimate the logarithmised odds-ratio and the logarithmised risk-ratio without applying any continuity correction.
...
Again, data input and argument specification works the usual way.
Just as with `rareMH` above, the use of Peto's method allows for estimation of the common log odds ratio without the use of any continuity correction, even in the presence of single- and double-zero studies.  

Let us use `rarePeto` on out data set and interpret its output.
...
In the following subsection, we aim to present functions allowing for pre-processing of the data at hand. 
...
The `rareDescribe` function computes descriptives on the 2x2 tables of the individual studies and stores them in a list. 
This list can then be used as data input into all of the other functions.
This is especially useful when the user wants to use more than one `raremeta` functions on the same data. 
Firstly, because it simplifies the input syntax and secondly, because this way, some of the computation is only done once and not at every call of an other `raremeta` function.

```{r, eval=FALSE}
rareDescribe(ai, bi, ci, di, n1i, n2i, data)
```

The data input works the usual way.  
Let us recall some of the examples above and see how the input is simplified.


## rareCC
The `rareCC` function enables the user to apply continuity correcion to the data.
Eventhough most functions allow to apply the correction internally, users may find it easier to pre-process the data this way and then input it to the other functions.
Since the functions above are set up in a way that a already continuity corrected data input will not be continuity corrected again, the user must not fear applying it twice.  
...
``` 
As always, data input can be done by specifying the `x` argument or the columns of the data frame at hand. 
We have implemented three different families of continuity correction: The *constant*, *treatment arm*, and *empirical* continuity correction.
...
By inputting vectors in the afforementioned arguments, different values can be added to different studies (adding the k-th value to the k-th study), making it possible for the user to introduce their own continuity correction.
...
When setting `cc = "empirical"`, the estimated summary effect size (`logOR, logRR` or `RD`) for all studies which enable the estimation of the corresponding individual effect size is used to calculate a value for continuity correction. 
This means that there must be at least one study enabling the estimation of the corresponding individual effect size and the measure- and method argument must be specified. 
For a precise definition (including an interpretation of the argument `ccsum`) see Sweeting et al. (2004).
This kind of continuity correction obviously depends on the effect size, which can be set by changing the `measure` argument.
Possible effect sizes are `"logOR`, `"logRR"`, and `"RD"`.
When it comes to model fitting, there is the possibility to fit fixed-effects models (also known as equal-effects models) and random-effects models using the inverse variance approach. 
A fixed-effects model is fitted when `method` is set to `"FE"` (or `"EE"`). A random-effects model is fitted when method is set to either `"DL", "HE", "SJ", "ML", "REML", "EB", "HS", "PM", "IPM", "GENQ", "PMM"` or `"GENQM"`. 
...
Let us go through some examples and see where how we can attain the continuity corrected data from the generated object. 
...
## rareES
To calculate individual effect sizes for meta-analysis of rare events, the `rareES` function can be used. 

```{r, eval=FALSE}
rareES(x,ai,bi,ci,di,n1i,n2i,data,measure,cc="none",ccval=0.5,tccval,cccval,ccsum=1,ccto="only0",drop00=TRUE,method="FE")
```

Data input is done in the usual way.
Through `measure` we may specify the effect size or outcome measure to be used (either `"logOR"` for the log odds ratio, ``"logRR"` for the log relative risk, or `"RD"` for the risk difference). 
The arguments from `cc` to `ccto` and `method` specify the continuity correction to be applied before calculation of the effect size estimate. 
Again, the argument `drop00` is a  logical indicating whether double-zero studies should be excluded prior to calculating the studies’ effect sizes and sampling variances.    

Let us have a look at the output of an application of the `rareES` function. 
We calculate the log odds ratio on the study level for the Yuan data set. 
```{r, eval=TRUE}
x <- rareES(ai=ai,ci=ci,n1i=n1i,n2i=n2i,data=dat.yuan2018,measure="logOR")
head(x)
```
The `rareES` function turned the Yuan data into an object of type `rareData`, adding the information about the estimated effect size and the inside study variances.
This is reflected by the output being a summary of all the information saved in this object.   
The output begins with a warning, stating that since we did not opt for the application of any continuity correction, the usual estimate of the log odds ratio for the single- and double-zero studies is undefined, therefore all of them were dropped in the analysis. 
Following that, there is a summary of the original data. 
After the counts of different kinds of studies follows a list of descriptive statistics of the total size of the the treatment group (group 1) and the control group (group 2), respectively.
The list consists of the mean, standard deviation, minimum, 25% quantile, median, 75% quantile, maximum and interquartile range.
Following that, we look at the same descriptive statistics for the sample size ratio, i.e. the size of the treatment group divided by the size of the control group.
It ends with analogue descriptives of the relative event-frequencies. 

The calculated estimates for the log odds ratio and the variance are added to the object and can be printed by referring to their names, `"yi` for the effect size estimates and `"vi"` for the variance estimates.
...
##### Effect size measures
The function includes methods for calculating log odds ratios, log risk ratios, and risk differences. The effect size measure can be specified using the measure argument. The respective effect size, along with an estimate of its sampling variance, is then calculated for each study based on the entries of the study's 2x2 table:
...
When either the log odds ratio or the log risk ratio is used as an effect size measure, both the effect sizes and their sampling variances are calculated based on the continuity-corrected 2x2 table. When the effect size measure is the risk difference, the continuity-corrected 2x2 table is only used in the calculation of the sampling variances.
...
##### Effect size measures
The function includes meta-analytic methods for log odds ratios, log risk ratios, and risk differences. The effect size measure can be specified using the measure argument. The respective effect size, along with an estimate of its sampling variance, is then calculated for each study based on the entries of the study's 2x2 table:
