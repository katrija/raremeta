...
on a data set by Hoppen et al. (2023).  
...
An example for such data is provided in a meta-analysis by Hoppen et al. (2023). 
In this meta-analysis, the authors evaluate the acceptability of psychological interventions, specifically Eye Movement Desensitization and Reprocessing (EMDR), for posttraumatic stress disorder (PTSD) in adults.
Specifically, they examine whether EMDR is associated with a larger (or smaller) number of dropouts compared to control treatments. 
A larger number of dropouts in EMDR indicates a lower acceptability of this treatment.  
...
In the Hoppen data, the treatment is EMDR.
This makes the treatment group consist of those individuals undergoing EMDR and the control group consist of those undergoing the control treatment.
...
The meta-analysis by Hoppen et al. is in fact an example for a meta-analysis of a rare event: Specifically, it contains one study with no event in one of the groups (single-zero study) and one study with no event in both groups (double-zero study).  
...
This may be done in `raremeta` by specifying the arguments `data, ai, bi, ci, di` or `data, ai, n1i, ci, n2i`.
We will go into more depth on the possibility to specifying the `x` argument when we talk about pre-processing the data by using the `rareDescribe` function later in this section.  
...
If we use the Hoppen data to fit a random effects meta-analysis on the logarithmised odds-ratio using the median-unbiased Paule-Mandel estimator while adding the constant value of 0.5 to all cells of all studies (as described in Viechtbauer 2021), we use the following input.
...
The output ends with the estimated model parameters, rounded to four decimal digits.
The logarithmised odd-ratio was estimated to be 0.5847419 (implying a estimated odds ratio of 1.794528).
The estimated effect size can be read from the list by way of the name `beta`.
```{r, eval = TRUE}
fitIV$beta
exp(fitIV$beta)
```
...
```{r, eval=FALSE}
rareMH(x,ai,bi,ci,di,n1i,n2i,data,measure,level=95,digits=4,correct=FALSE,cc="constant",ccval=0.5,
       tccval,cccval,ccsum=1,ccto="only0",method="FE")
```
...
```{r, eval=FALSE}
rareES(x,ai,bi,ci,di,n1i,n2i,data,measure,cc="constant",ccval=0.5,tccval,cccval,ccsum=1,
...
```{r, eval=TRUE}
x <- rareES(ai=ai,ci=ci,n1i=n1i,n2i=n2i,data=dat.hoppen2023,measure="logOR")
x
```
The `rareES` function turned the Hoppen data into an object of type `rareData`, adding the information about the estimated effect size and the inside study variances.
...
After the counts of different kinds total number of studies and number of single- (double-) zero studies follows a list of descriptive statistics of the total size of the the treatment group (group 1) and the control group (group 2), respectively.
