...
fitMH
```
The output begins with a short description of the applied method and follows up with the number of studies which were part of the analysis.
For model results, the estimated logarithmised relative risk, standard error, z-val, p-val and lower- and upper bound of the confidence interval are stated. 
The estimated relative risk amounts to 
```{r, eval=TRUE}
exp(fitMH$b)
```
which indicates that on average the risk of dropout are about $1.44$ times as large in EMDR groups as compared to passive control groups.

The standard error associated with the pooled log risk ratio is estimated at $0.1942$. 
The Wald test for which the test statistic and the $p$-value are reported ($z = 1.8740$, $p = 0.0609$) indicates that the treatment effect is not significantly different from zero. 
This information can also be obtained from the confidence interval $(-0.0167, 0.7447)$, which overlaps $0$.
...
fitPeto
```
Just as before, the output states a short description of the used model, the number of studies and the model results.  
...
The standard error associated with the pooled log relative risk is estimated at $0.2386$. 
The Wald test for which the test statistic and the $p$-value are reported ($z = 1.9092$, $p = 0.0562$) indicates that the treatment effect is not significantly different from zero. 
This information can also be obtained from the confidence interval $(-0.0121, 0.9233)$, which overlaps $0$.
...
# Pre-processing of meta-analytic rare event data
...
Let us recall the examples above and see how the input is simplified, i.e. the following calls are equivalent to the calls seen in the three subsections above.
...
Let us go through some examples and see  how we can attain the continuity corrected data from the generated object.  
We begin with the classical continuity correction of adding $0.5$ to all cells of all studies with zero events in one of the groups.

```{r, eval=TRUE}
x <- rareCC(ai=ai,ci=ci,n1i=n1i,n2i=n2i,data=dat.hoppen2023,
            cc="constant",ccval=0.5,ccto="only0",drop=TRUE)
data.frame(x$ai,x$ci,x$n1i,x$n2i)
data.frame(x$ai.cc,x$ci.cc,x$n1i.cc,x$n2i.cc)
```

As one can see from the output above, it is possible to call for the values of the original data via the column names and the values of the continuity corrected data via the column names with an added `.cc`.
Since we opted for `drop00=TRUE` and `ccto="only0"`, the double-zero study was excluded and the single-zero study was continuity corrected.  

If we want apply the `empirical` continuity correction to all studies, not excluding the double-zero studies, we call for:
```{r, eval=TRUE}
x <- rareCC(ai=ai,ci=ci,n1i=n1i,n2i=n2i,data=dat.hoppen2023,
